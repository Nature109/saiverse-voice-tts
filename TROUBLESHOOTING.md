...
**最初に確認**: アドオン管理 UI で「クライアント側再生」と「サーバー側再生」のどちらが ON か。

| 設定 | 期待される動作 |
|---|---|
| クライアント側再生 ON (既定) | アクティブなブラウザタブから音が鳴る |
...
> v2.0 以降、**サーバー側再生の既定値は OFF** です。ブラウザ経由で鳴らす「クライアント側再生」が既定になりました。バックエンド PC のスピーカーから直接鳴らしたい場合はこのセクション。
...
| デバイスが「リモート オーディオ」 | アドオン UI の「出力オーディオデバイス」で実デバイスを選択 |
...
| クライアント側再生のみ途切れる | ネットワーク帯域不足(Tailscale 等) | ストリーミング OFF で完了後再生 |
...
**動作**: クライアント側再生 ON の場合、**最後に画面を触った端末**が自動的に「アクティブクライアント」となり、その端末だけで音が鳴ります。
...
> **v2.0 からは RDP 不要**: クライアント側再生で Tailscale 経由のブラウザから直接音を鳴らせます。RDP は「サーバー側再生を RDP クライアント PC で聞きたい」ケースでのみ使用。
...
# 2. アドオン管理 UI の「出力オーディオデバイス」で該当デバイスを選択
# 3. 「サーバー側再生」を ON に
```
...
**症状**: モバイル表示で「出力オーディオデバイス」「参照音声の書き起こし」等のラベルが 1 文字ずつ縦に折れる。
