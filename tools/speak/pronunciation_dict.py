...
- ``voice_profiles/pronunciation_dict.json`` (ユーザー編集ローカル、.gitignore)
...
適用順序は **persona override → global → そのまま**。
"""
...
# (key, value) の長さ降順ソート済みリスト。グローバル辞書のキャッシュ。
_cached_global_entries: Optional[List[Tuple[str, str]]] = None
_lock = Lock()
...
def _load_global_entries() -> List[Tuple[str, str]]:
    """グローバル辞書をロード(キャッシュあり)。"""
    global _cached_global_entries
    if _cached_global_entries is not None:
        return _cached_global_entries
    with _lock:
        if _cached_global_entries is not None:
            return _cached_global_entries
        ...
        try:
            ...
            _cached_global_entries = []
            return _cached_global_entries
        _cached_global_entries = _normalize_entries(data)
        LOGGER.debug(
            "Loaded %d pronunciation dict entries from %s",
            len(_cached_global_entries), source.name,
        )
    return _cached_global_entries


def reload() -> None:
    """辞書ファイルを再読込(編集後にバックエンド再起動なしで反映する用)。"""
    global _cached_global_entries
    with _lock:
        _cached_global_entries = None
