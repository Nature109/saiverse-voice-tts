...
#### ペルソナ別オーバーライド (推奨: アドオン管理 UI)

SAIVerse 本体の **アドオン管理 → Voice TTS → ペルソナ別設定 → 読み方辞書**
で key/value を直接追加・編集できる。本体側に `dict` 型の params_schema が
入った後 (本体 `feature/addon-dict-param-type` のマージが必要) のバージョン
から有効。`voice_profiles/registry.json` の `pronunciation_dict` キーよりも
UI 側の値が優先される。

CLI で一括管理したい場合は従来通り `registry.json` のペルソナエントリに
`pronunciation_dict` キーを書く方式も併用可能:
...
- `tools/speak/profiles.py`:
  - AddonPersonaConfig 経由の場合も `pronunciation_dict` を profile の
    top-level に引き上げる(engine の params に混じらないよう除外)
  - registry ルート時にも UI 設定の `pronunciation_dict` をマージする
    (`_try_get_ui_pronunciation_dict` ヘルパー追加)
- `addon.json`: `pronunciation_dict` (type=dict, persona_configurable=true)
  をペルソナ別設定に追加
- `voice_profiles/pronunciation_dict.json.template` (新規)
- `.gitignore`: ローカル辞書を追記
- `tests/test_pronunciation_dict.py` (新規): 15 件のユニットテスト
- `tests/test_profiles.py` (新規): UI/registry マージ挙動の 11 件ユニットテスト
