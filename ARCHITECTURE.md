...
#### `qwen3_tts.py`
`generate_voice_clone` で Base バリアント(Qwen/Qwen3-TTS-12Hz-1.7B-Base)を使う。ストリーミング非対応。参考用のエンジン。

#### `irodori.py`
未検証。`InferenceRuntime.synthesize()` のラッパスケルトン。
...
5. **transformers バージョン競合**: Qwen3-TTS と GPT-SoVITS は共存不可
