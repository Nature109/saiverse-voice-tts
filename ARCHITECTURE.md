...
### 本体アドオン基盤(SAIVerse feature/memory-notes-and-organize, 2026-04)
...
- `addon.json` マニフェストによる UI 拡張(バブル内ボタン自動生成)
...
| `api_routes.py` 自動マウント | `/api/addon/saiverse-voice-tts/audio/*` |

### ストリーミング配信チャンクの流れ(addon 連携版)

```
synthesize_stream() のチャンク
   │
   ├─ sd.OutputStream.write(chunk)          ← サーバ再生(既存)
   ├─ audio_stream.push_chunk(msg_id, ...)  ← HTTP Chunked にも同時投入
   │
   (すべて完了)
   │
   ├─ wav 保存
   ├─ set_metadata(msg_id, audio_path=...)
   └─ emit_addon_event("audio_ready")       ← フロントのバブル再生ボタン有効化
```
