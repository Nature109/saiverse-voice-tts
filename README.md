...
- **`num_steps`**: 既定 24、高品質目的なら 32。大きいほど音質が上がるが合成時間も増える
- **`truncation_factor`**: 既定 0.75(パックでハードコード、下げ過ぎると文途切れ)。低確率トークンを除外して「予算埋めゴミ音声」を抑制する重要パラメータ
- **`seconds`(内部自動算出)**: チャンクごとに文字数から推定(`chars * 0.25 + 1.5` 秒)。長すぎるとゴミ末尾、短すぎると本文途切れのトレードオフを回避
- **`model_precision`**: 既定 `bf16`(CUDA 必須)。fp32 にすれば安定するが 3 倍遅くなる
- **`codec_device` / `codec_precision`**: 既定 `cuda` / `bf16`。CPU にすると codec decode が律速になり RTF が 5 倍以上悪化する
...
アドオン UI でアップロードするのが楽ですが、CLI で一括管理したい場合は `voice_profiles/registry.json` にペルソナ ID をキーとしてエントリを追加:
