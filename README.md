...
SAIVerse 拡張パック。ペルソナが発話したテキストを、ペルソナごとに登録した参照音声でゼロショット音声クローン合成し、バックエンドPCのスピーカーから自動再生します。**2 回目以降は話し始めまで 5 秒以内**(環境により前後)。初回発話はモデルロードのため数十秒かかります。

## 特徴

- **ゼロショット音声クローン** — 3秒以上の参照音声1つで、そのペルソナの声を再現
- **ストリーミング推論** — 合成完了を待たずチャンク単位でリアルタイム再生(体感レイテンシを大幅短縮)
- **SAIVerse アドオン基盤対応** — メッセージバブルに🔊再生ボタンが自動表示。アドオン管理 UI からペルソナごとに自動発話の ON/OFF や再生モード切替が可能
...
- **HTTP Chunked 配信対応** — リモート(Tailscale)クライアントへのストリーミング配信ルートを用意 ※フロント対応は本体側のロードマップ
...
**本体バージョンについて**: バブルの🔊再生ボタン・アドオン管理 UI 連携を有効化するには、SAIVerse 本体が以下の API/モジュールを提供している必要があります:
...
これらが存在しない旧バージョンでも、バックエンドPC のスピーカーから自動再生する基本機能のみ動作します(フロントの再生ボタン・リモート配信は機能しない)。
...
> Qwen3-TTS はストリーミング非対応で話し始めに 30 秒程度かかり実用性に欠けるため、本パックからはサポートを外しました(コミット履歴には残っています)。必要な場合は過去コミット `44a7d42` 以前を参照してください。
